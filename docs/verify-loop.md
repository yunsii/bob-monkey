...
```bash
pnpm verify list
pnpm verify toggle "bob-monkey" off
```
...
# 断言具体行为
pnpm verify eval deepwiki.com "getComputedStyle(document.querySelector('[data-bob-monkey-shadow-root]')).zIndex"

# 改完代码之后
pnpm verify reload deepwiki.com
pnpm verify wait deepwiki.com "..."
```
...
pnpm verify toggle "bob-monkey" on       # 把之前关掉的开回来
```
...
```bash
pnpm verify remove "bob-monkey (local build)"
...
- **这份文档里的 `bob-monkey-*` 不是写死的**。DOM 属性、元素 id、自定义元素名全都从
  `src/helpers/namespace.ts` 的 `NAMESPACE` 推导，改过那个常量之后照抄这里的命令会一个
  都查不到，症状恰好和「脚本没装上」一模一样。对不上时先看一眼那个常量。
