...
重命名成 `--bm-tw-*`（前缀取自 `helpers/namespace.ts`），避免和宿主页面自己的 Tailwind 撞车。
...
容器同时带 `z-index: 2147483647`。`position: fixed` 在解决裁剪的同时也创建了层叠上下文，于是这个
容器变成一道墙：**里面的弹层写多大的 `z-index` 都只在墙内有效**，整棵子树以容器自己的 `z-index`
参与外层排序。容器不写就是 `auto`，落在「已定位、`z-index: auto`」那一层，页面上任何
`z-index > 0` 的元素都画在它之后 —— 也就是盖在它上面。

实测：页面一个 `sticky top-0 z-30` 的头部，压住了容器里 `z-index: 2147483000` 的模态遮罩。遮罩的
矩形是满视口的 `[0, 0, 1886, 906]`，几何上没问题，纯粹是绘制顺序输了；`30` 打得过 `2147483000`，
因为两者根本不在同一个层叠上下文里比。

`inline` 宿主尤其要紧。`overlay` / `modal` / `detached` 的宿主自带 `z-index: 2147483647`，就算容器
不写也有宿主兜着；而 `inline` 宿主按定义留在文档流里、不带 `z-index`（它的选项类型里也没有这个
字段），指望不上。容器自己写死上限之后，四种 `position` 就一致了。
...
```bash
pnpm verify eval "deepwiki.com" "getComputedStyle(document.querySelector('deepwiki-links')).zIndex"
```
...
pnpm verify eval "deepwiki.com" "
  (() => {
    const sr = document.querySelector('bob-monkey-settings').shadowRoot
...
pnpm verify eval "deepwiki.com" "
  (async () => {
    ...
    const btn = document.querySelector('bob-monkey-settings').shadowRoot.querySelector('.ant-btn')
