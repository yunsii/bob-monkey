...
describe('rewriteUserscriptMeta', () => {
  it('用产物里的 @name 而不是包名作为基名', () => {
    ...
    assert.equal(baseName, 'bob-monkey')
    assert.equal(localName, 'bob-monkey (local build)')
  })
  ...
  it('改掉所有 @name 本地化变体', () => {
    ...
    assert.deepEqual(new Set(names), new Set(['bob-monkey (local build)']))
  })
  ...
  it('换掉 @namespace，让本地构建与已发布版并存而不是覆盖', () => {
    const { source } = rewriteUserscriptMeta(SAMPLE)
    assert.match(source, new RegExp(`^// @namespace\\s+user/${PKG_NAME}-local-build$`, 'm'))
...
describe('真实构建产物', () => {
  const bundle = join(ROOT, 'dist', `${PKG_NAME}.user.js`)
