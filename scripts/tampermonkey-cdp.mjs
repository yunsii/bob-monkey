 ...
 *   node scripts/tampermonkey-cdp.mjs toggle "bob-monkey" off
 *   node scripts/tampermonkey-cdp.mjs remove "bob-monkey (local build)"
 *   node scripts/tampermonkey-cdp.mjs cleanup
 *   node scripts/tampermonkey-cdp.mjs open https://deepwiki.com/facebook/react
 *   node scripts/tampermonkey-cdp.mjs reload deepwiki.com
 *   node scripts/tampermonkey-cdp.mjs eval deepwiki.com "document.title"
 *   node scripts/tampermonkey-cdp.mjs wait deepwiki.com "!!document.querySelector('[data-bob-monkey-shadow-root]')"
 *   node scripts/tampermonkey-cdp.mjs type deepwiki.com "hello"
...
// 本地构建用的 `@namespace` 从包名派生，而不是写死某个仓库名：写死的话 fork 之后
// 两个不同项目的本地构建会共用同一个 namespace，Tampermonkey 认成同一个脚本。
  ...
  const source = raw
    ...
    .map((line) => {
      ...
      return /^\/\/ @namespace\s/.test(line)
        ? `// @namespace    user/${PKG_NAME}-local-build`
        : line
    })
    .join('\n')
